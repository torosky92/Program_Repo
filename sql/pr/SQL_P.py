...
class SQLP(Base):
   ...
   def UpdateP(TABLA: str, REMISION: str, DATO_MOD, Value):
      ...
      if DATO_MOD == Settings.Var_Comp13(): session.query(SQLP).filter_by(Ref=valueRef).update({SQLP.Ref: Value})
      elif DATO_MOD == Settings.Var_Comp22(): session.query(SQLP).filter_by(CodeC=ValueCodeC).update({SQLP.CodeC: Value})
      elif DATO_MOD == Settings.Var_Comp27(): session.query(SQLP).filter_by(Weight_Tare=ValueWeight_Tare).update({SQLP.Weight_Tare: Value})
      elif DATO_MOD == Settings.Var_Comp8(): session.query(SQLP).filter_by(Weight=ValueWeight).update({SQLP.Weight: Value})
      elif DATO_MOD == Settings.Var_Comp28(): session.query(SQLP).filter_by(Weight_Desp=ValueWeight_Desp).update({SQLP.Weight_Desp: Value})
      elif DATO_MOD == Settings.Var_Comp29(): session.query(SQLP).filter_by(Weight_Desp_Tare=ValueWeight_Desp_Tare).update({SQLP.Weight_Desp_Tare: Value})
      session.commit()
      session.close()

   def CreateP(TABLA: str):
      engine = create_engine(TABLA, echo=True)
